...
from uc.uc_block import CFG, format_instruction, EmitBlocks
...
class DataFlow(NodeVisitor):
    ...
    def short_circuit_jumps(self):
        prev_inst = self.code[0]
        for i, inst in enumerate(self.code):
            if inst[0] == 'jump':
                prev_inst = inst
            elif ('%' + inst[0]) == (prev_inst[1] + ':'):
                self.code.pop(i-1)

    def appendOptimizedCode(self, block):
        bb = EmitBlocks()
        bb.visit(block)
